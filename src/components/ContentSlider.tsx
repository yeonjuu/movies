...
interface IMovieProps {
  data: [string, IMovie[]] | [string, ITvShow[]] | [string, ITrendingTvShow[]];
  onBoxClicked: (movieId: number) => void;
  haveRanking?: boolean; // 랭킹이 있는지 여부
}
...
//이름 추천
export const ContentSlider = ({ data, onBoxClicked, haveRanking }: IMovieProps) => {
  ...
  const maxIndex = movies.length - offset; // offset * 6은 한 번에 보여줄 영화의 개수
    ...
    <Slider>
      <AnimatePresence initial={false} onExitComplete={toggleLeaving}>
        <React.Fragment key={type}>
          <LeftDiv>
            <StyledButton className="left" onClick={decreaseIndex} disabled={disabled.left}>
              <LeftArrowSvg />
            </StyledButton>
          </LeftDiv>
          <RightDiv>
            <StyledButton className="right" onClick={increaseIndex} disabled={disabled.right}>
              <RightArrowSvg />
            </StyledButton>
          </RightDiv>
        </React.Fragment>
        ...
        >
          {movies
            ...
            .map((movie, i) => {
              ...
              return (
                <Poster key={`${type}-${movie.id}`}>
                  {
                    // 랭킹이 있는 경우, 랭킹 아이템을 표시
                    haveRanking && (
                      <RankingItem>
                        <RankingNumber>{absoluteIndex + 1}</RankingNumber>
                      </RankingItem>
                    )
                  }
                  <Box
